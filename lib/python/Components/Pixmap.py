		...
		if self.skinAttributes is not None:
			...
			for (attrib, value) in self.skinAttributes:
				if attrib == "pixmaps":
					pixmaps = value.split(',')
					for p in pixmaps:
						pngfile = resolveFilename(SCOPE_GUISKIN, p, path_prefix=skin_path_prefix)
					...
					if not pixmap:
						...
						elif fileExists(resolveFilename(SCOPE_LCDSKIN, pixmaps[0], path_prefix=skin_path_prefix)):
							pixmap = resolveFilename(SCOPE_LCDSKIN, pixmaps[0], path_prefix=skin_path_prefix)
				elif attrib == "pixmap":
					pixmap = resolveFilename(SCOPE_GUISKIN, value, path_prefix=skin_path_prefix)
					...
					elif fileExists(resolveFilename(SCOPE_LCDSKIN, value, path_prefix=skin_path_prefix)):
						pixmap = resolveFilename(SCOPE_LCDSKIN, value, path_prefix=skin_path_prefix)
				else:
