...
class FileList(MenuList):
	def __init__(self, directory, showDirectories=True, showFiles=True, showMountpoints=True, matchingPattern=None, useServiceRef=False, inhibitDirs=False, inhibitMounts=False, isTop=False, enableWrapAround=False, additionalExtensions=None):
		...
		if matchingPattern:
			self.matchingPattern = re_compile(matchingPattern)
		else:
			...
			if not x[0][3].startswith('<'):
				...
				if x[0][2]:
					...
					try:
						...
						try:
							self.selectedFiles.remove(os_path.normpath(realPathname))
						except:
							print("Couldn't remove:", realPathname)
				else:
					SelectState = True
					if (realPathname not in self.selectedFiles) and (os_path.normpath(realPathname) not in self.selectedFiles):
	...
	def changeDir(self, directory, select=None):
		...
		if self.showDirectories:
			for x in directories:
				if not (self.inhibitMounts and self.getMountpoint(x) in self.inhibitMounts) and not self.inParentDirs(x, self.inhibitDirs):
					name = x.split('/')[-2]
					alreadySelected = (x in self.selectedFiles) or (os_path.normpath(x) in self.selectedFiles)
