...
# even more shamelessly modified by datasox
...
class PliExtraInfoNobile(Poll, Converter, object):
	...
	def createPIDInfo(self,info):
		...
		return "V:%04d A:%04d PCR:%04d SID:%05d" % (vpid,apid,pcrpid,sidpid)
	...
	def getText(self):
		...
		if self.type == "All":
			...
			if config.usage.show_cryptoinfo.value:
				...
				+ addspace(self.createTransponderInfo(fedata,feraw)) + "\n"\
				+ addspace(self.createPIDInfo(info)) + "\n"\
				+ addspace(self.createResolution(info) + " - " + self.createVideoCodec(info))
			else:
				return addspace(self.createCryptoBar(info)) + "\n"\
				+ self.current_source + "\n"\
				+ addspace(self.createTransponderInfo(fedata,feraw)) + "\n"\
