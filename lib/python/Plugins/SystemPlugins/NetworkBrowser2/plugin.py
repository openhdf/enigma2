...
class MountAgainCheckPoller:
    ...
    def onTimer(self):
        ...
        if config.networkbrowser.automountpoll.value:
            ...
            self.timer.startLongTimer(1800)
...
def Plugins(path, **kwargs):
    ...
    return [PluginDescriptor(where=[PluginDescriptor.WHERE_AUTOSTART, PluginDescriptor.WHERE_SESSIONSTART], fnc=autostart),
     PluginDescriptor(name=_('Network Browser'), description=_('Search for network shares'), where=PluginDescriptor.WHERE_NETWORKMOUNTS, fnc={'ifaceSupported': NetworkBrowserCallFunction,
      'menuEntryName': lambda x: _('Network Browser'),
      'menuEntryDescription': lambda x: _('Search for network shares...')}),
     PluginDescriptor(name=_('Mount Manager'), description=_('Manage network shares'), where=PluginDescriptor.WHERE_NETWORKMOUNTS, fnc={'ifaceSupported': MountManagerCallFunction,
      'menuEntryName': lambda x: _('Mount Manager'),
      'menuEntryDescription': lambda x: _('Manage your network shares...')}),
     PluginDescriptor(name=_('Mount Again'), description=_('Attempt to mount shares again'), where=PluginDescriptor.WHERE_NETWORKMOUNTS, fnc={'ifaceSupported': RemountCallFunction,
