...
# fix me to xz
	...
	def read_current_multiboot(self):
		...
		elif self.MACHINEBUILD in ("cc1","sf8008","sf8008s","sf8008t"):
			if self.list[self.selection] == "Recovery":
	...
	def doFullBackup(self, DIRECTORY):
		...
		elif "tar.bz2" in self.ROOTFSTYPE.split() or SystemInfo["HaveMultiBoot"] or self.MACHINEBUILD in ("u51","u52","u53","u5","u5pvr","sf8008","sf8008s","sf8008t"):
			self.MKFS = "/bin/tar"
		...
		self.message = "echo -e '"
		self.message += _("Backup Tool for") + ' %s' %self.SHOWNAME + " - " + VERSION + '\n'
		#self.message += 
...
#			cmd1 = "%s -cJf %s/root.tar.xz -C /tmp/bi/root ." % (self.MKFS, self.WORKDIR)
			...
			cmd2 = None
			cmd3 = None
		elif "tar.bz2" in self.ROOTFSTYPE.split() or SystemInfo["HaveMultiBoot"] or self.MACHINEBUILD in ("u51","u52","u53","u5","u5pvr","cc1","sf8008","sf8008s","sf8008t"):
	...
	def doFullBackupCB(self):
		...
		if self.DIRECTORY == "/hdd":
			...
			if self.TARGET == 'XX':
				...
				cmdlist.append('echo "' + _("back-up image will now be copied to that USB-flash drive. ") + '"')
