...
def SkinSelMain(session, **kwargs):
	session.open(VFDSkinSelector)

def SkinSelSetup(menuid, **kwargs):
	if menuid == "system":
		return [(_("VFD Skin Selector"), SkinSelMain, "vfdskin_selector", 60)]
	else:
		return []

def Plugins(**kwargs):
	return PluginDescriptor(name="VFDSkinselector", description="VFD Skin-Selector for GigaBlue and VU+", where = PluginDescriptor.WHERE_MENU, needsRestart = False, fnc=SkinSelSetup)
